...
utils = require('./utils');
 ...
 function Anubis() {
  ...
  roundKeyDec = [];
	...
	function rot(value, r) {
		if (r === 0) return value;
		if (r > 0) {
			let result = value >> r;
			if ((Math.pow(2, 53)+'').length === 8){
        return result;
      }
  ...
  function keySetup(key) {
	  ...
	  R = 8 + N;
		...
		for (i = 0, pos = 0; i < N; i++) {
			kappa[i] =
			((utils.ord(key[pos++])       ) << 24) ^
			((utils.ord(key[pos++]) & 0xff) << 16) ^
			((utils.ord(key[pos++]) & 0xff) <<  8) ^
			((utils.ord(key[pos++]) & 0xff)      );
		}

		for (r = 0; r <= R; r++) {
			let K0 = T4[rot(kappa[N - 1], 24)       ],
			K1 = T4[rot(kappa[N - 1], 16) & 0xff],
			K2 = T4[rot(kappa[N - 1],  8) & 0xff],
	...
	function crypt(block, roundKey) {
		...
		for (i = 0, pos = 0; i < 4; i++) {
			state[i] =
				((utils.ord(block[pos++])       ) << 24) ^
				((utils.ord(block[pos++]) & 0xff) << 16) ^
				((utils.ord(block[pos++]) & 0xff) <<  8) ^
				((utils.ord(block[pos++]) & 0xff)      ) ^
				roundKey[0][i];
		}
  ...
  function encrypt(key, data, obj) {

    try {
      let def = ['iv', 'buffer','iterations', 'hash'],
      mac;
      obj = obj || {};
      ...
      if(obj.pbkdf2 && !obj.salt){
        final.err = true;
        final.data = 'no salt detected'
        return final;
      }
      if(obj.pbkdf2 && obj.salt){
        key = keySetup(
          crypto.pbkdf2Sync(key, obj.salt, obj.iterations, 16, obj.hash).toString('hex')
        );
      } else {
        key = keySetup(key);
      }

      let blocks = utils.dPrep(data),
      register = obj.iv,
      cypher = register;
      ...
      if(obj.mac && obj.mac !== ''){
        mac = utils.hmac(obj.mac,utils.enc2hex(cypher), obj.hash);

        if(obj.buffer){
          mac += utils.enc2hex(cypher);
          final.data = utils.rand(obj.buffer[0]) + mac + utils.rand(obj.buffer[1])
          return final;
        }
        mac += utils.enc2hex(cypher);
        final.data = mac;
      ...
      if(obj.buffer){
        final.data = utils.rand(obj.buffer[0]) + utils.enc2hex(cypher) + utils.rand(obj.buffer[1])
        return final;
      }

      final.data = utils.enc2hex(cypher);
      return final;
    } catch (err) {
      final.err = true;
      final.data = 'anubis encrypt error'
      console.log(err)
      return final
    }
   ...
   function decrypt(key, data, obj) {

     try {
       let def = ['buffer','iterations', 'hash'],
       mac;
       obj = obj || {};
       ...
       if(obj.pbkdf2 && obj.salt){
        ...
        if(obj.buffer){
          data = data.slice(obj.buffer[0] * 2, -(obj.buffer[1] * 2))
        }

        if(obj.mac && obj.mac !== ''){
          mac = utils.substr(data, 0,128);
          let check = utils.hmac(obj.mac, data.slice(128), 'sha512');
          if(mac !== check){
            final.err = true;
            final.data = 'data tamper detected'
            return final;
          }
        }

        data = utils.decHex(data.slice(128));

        let blocks = utils.dPrep(data, false),
        //first block is Initialization Vector (IV)
        register = blocks.shift();
        let decrypted = '';
        for(var i=0; i < blocks.length; i++) {
          let crypted = crypt(blocks[i], roundKeyDec);
        ...
        decrypted = utils.substr(decrypted, 0, -utils.ord(utils.substr(decrypted, -1, 1)));
      ...
      if(err) {
        final.err = true;
        final.data = 'anubis decrypt error'
        console.log(err)
        return final
      }
     }
	...
	return {
    encSync: function(key, data, obj){
      res = encrypt(key, data, obj)
      if(!res.err){return res.data;}
      return res;
    },
    decSync: function(key, data, obj){
      res = decrypt(key, data, obj);
      if(!res.err){return res.data;}
      return res;
    },
    enc: function(key, data, obj, cb){
